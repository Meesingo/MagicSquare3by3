...
verbose_Input = input('Do you want verbose? (\'y\' for yes and anything else for no): ')
output_Input = input('Do you want output? (\'y\' for yes and anything else for no): ')
complete_Input = input('Do you want all solutions? (\'y\' for yes and anything else for no): ')
# If there are arguments, proceed with arguments
if verbose_Input.lower() in ['y', '\'y\'', 'yes']:
    value_Global.verbose = True
if output_Input.lower() in ['y', '\'y\'', 'yes']:
    value_Global.output = True
    value_Global.output_File = open('Magic_Square_v2.log.txt', 'w')
if complete_Input.lower() in ['y', '\'y\'', 'yes']:
